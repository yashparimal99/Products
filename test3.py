...
def manage_accounts():
    ...
    # --- Approved or rejected requests for display ---
    cur.execute("SELECT * FROM accounts_requests WHERE status_flag IN ('A', 'R')")
    processed_requests = cur.fetchall()
...
def viewcards():
    ...
    # 🔹 Fetch pending requests (status_flag is NULL)
    cur.execute("""
        SELECT r.request_id, r.customer_name, r.customer_email, r.card_type, r.card_subtype,
               r.application_ref, r.created_at, r.status_flag, r.submitted_by_role,
               r.date_of_action, a.id AS app_id
    ...
    cur.execute("""
        ...
        LEFT JOIN card_applications a
          ON r.application_ref = a.application_ref
        WHERE r.status_flag IN ('A', 'R')
        ORDER BY r.date_of_action DESC
    """)
    processed_requests = cur.fetchall()
    cur.close()
...
def view_cards():
    ...
    cur.execute("""
        ...
        WHERE (customer_user_id=%s OR submitted_by_user_id=%s) AND status_flag = 'A'
        ORDER BY created_at DESC, id DESC
    """, (user_id, user_id))
    rows = cur.fetchall()
    cur.close()
...
def userdashloan():
    ...
    cur.execute("SELECT * FROM loan_requests WHERE request_id=%s ORDER BY application_date DESC", (user_id,))
    loans = cur.fetchall()

    # Map status to friendly labels
    for loan in loans:
        loan['status_label'] = _status_label(loan.get('status'))

    cur.close()
    return render_template('userloand.html', user=user, loans=loans)
...
def _status_label(s):
    return {
        ...
        'approved_docs':'Documents Approved',
        'approved': 'Approved',
        'rejected': 'Rejected',
        'issued': 'Issued'
    }.get((s or '').lower(), s or '—')
...
def save_all_files_as_zip(applicant_name, request_id, files):
    folder = 'uploads/home_loan_docs'
    os.makedirs(folder, exist_ok=True)
    zip_filename = f"{folder}/{applicant_name}_{request_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.zip"
...
def find_interest_rate(loan_type, interest_type, amount, tenure):
    if loan_type == 'home':
        if interest_type.lower() == 'floating':
            return 8.50
        elif interest_type.lower() == 'fixed':
            return 9.00
    return None
...
def homeloan():
    ...
    try:
        mysql.connection.ping(True)  # keep MySQL connection alive
        ...
        # Convert to Decimals for DB safety but floats for EMI computation
        co_name = f.get('coApplicantName') or None
        co_income = safe_decimal(f.get('coApplicantIncome')) if f.get('coApplicantIncome') else None
        gross_income = safe_decimal(f.get('grossIncome'))
        loan_amount = safe_decimal(f.get('loanAmount'))
        property_value = safe_decimal(f.get('propertyValue'))
        loan_tenure = int(f.get('loanTenure') or 0)
        interest_type = f.get('interestType')
        loan_purpose = f.get('loanPurpose')

        # Interest rate and EMI
        rate = find_interest_rate('home', interest_type, float(loan_amount), loan_tenure)
        if rate is None:
            rate = 8.50 if interest_type.lower() == 'floating' else 9.00

        # convert Decimal → float before EMI calc
        emi = compute_emi(float(loan_amount), float(rate), loan_tenure)

        with mysql.connection.cursor(MySQLdb.cursors.DictCursor) as cur:
            # Insert into home_loan_applications
            cur.execute("""
                INSERT INTO home_loan_applications (
                    request_id, user_id, applicant_name, coapplicant_name, 
                    applicant_annual_income, coapplicant_annual_income, 
                    gross_monthly_income, loan_amount, property_value, 
                    interest_type, loan_tenure, loan_purpose, 
                    interest_rate, estimated_emi, documents_zip_path, status
                )
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                request_id, user['cust_id'], applicant_name, co_name,
                gross_income, co_income, gross_income / Decimal(12), loan_amount,
                property_value, interest_type, loan_tenure, loan_purpose,
                rate, emi, docs_zip_path, 'Pending'
            ))

            # Insert into loan_requests
            cur.execute("""
                INSERT INTO loan_requests (
                    request_id, cust_id, loan_type, amount, tenure_years, interest_rate, 
                    emi, interest_type, status, created_at
                ) 
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())
            """, (
                request_id, user['cust_id'], 'Home Loan', loan_amount, loan_tenure,
                rate, emi, interest_type, 'Pending'
            ))

        mysql.connection.commit()

        flash(f'🏠 Home Loan Application Submitted Successfully! '
              f'Interest Rate: {rate:.2f}% | EMI: ₹{emi:.2f}', 'success')
...
def view_loans():
    ...
    def _fetch_and_append(kind, table):
        ...
        cur.execute(f"""
            ...
            FROM {table}
            WHERE user_id=%s AND status IN ('approved', 'approved_docs')
        ...
        for r in rows:
            ...
            loans.append({
                'loan_kind': kind,
                'loan_type_label': 'Home Loan' if kind == 'home' else (
                    'Personal Loan' if kind == 'personal' else 'Business Loan'),
...
@app.route('/loan_verification')
def loan_verification():
    """
    Display only pending loan requests for admin/staff verification.
    """
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute("""
    ...
    loans = cur.fetchall()
    cur.close()
    return render_template("verification.html", loans=loans)
